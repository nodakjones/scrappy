#!/usr/bin/env python3
"""
Show the most recently processed contractors from our last batch test.
"""
...
async def show_recent_results():
    """Show the most recently processed contractors"""
    print("📊 MOST RECENTLY PROCESSED CONTRACTORS")
    print("=" * 60)
    ...
    try:
        # Get contractors processed in the last hour (our recent batch test)
        ...
        if not result:
            print("❌ No recently processed contractors found!")
            print("Trying to get all completed contractors...")
        ...
        if not result:
            print("❌ No processed contractors found at all!")
            return
        
        print(f"📋 Found {len(result)} recently processed contractors")
        print(f"⏰ Time range: Last hour (since {one_hour_ago.strftime('%H:%M:%S')})")
        print()
        ...
        for row in result:
            ...
            processed_time = row[8] or "Unknown"
            ...
            if isinstance(processed_time, datetime):
                processed_str = processed_time.strftime('%H:%M:%S')
            else:
...
if __name__ == "__main__":
    asyncio.run(show_recent_results()) 