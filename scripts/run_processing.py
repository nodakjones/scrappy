...
class ProcessingOrchestrator:
    ...
    async def export_processed_contractors(self, limit: int = None):
        """Export processed contractors to CSV"""
        logger.info("Starting contractor export...")
        
        # Get exportable contractors
        contractors = await self.export_service.get_exportable_contractors(limit=limit)
        
        if not contractors:
            logger.warning("No contractors ready for export")
            return None
        
        logger.info(f"Exporting {len(contractors)} contractors")
        
        # Create both full and summary exports
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Full export
        full_filename = f"contractor_export_full_{timestamp}.csv"
        full_path = await self.export_service.export_to_csv(contractors, full_filename)
        
        # Summary export
        summary_filename = f"contractor_export_summary_{timestamp}.csv"
        summary_path = await self.export_service.create_summary_export(contractors, summary_filename)
        
        self.exported_count = len(contractors)
        
        logger.info(f"Export completed:")
        logger.info(f"  - Full export: {full_path}")
        logger.info(f"  - Summary export: {summary_path}")
        logger.info(f"  - Records exported: {len(contractors)}")
        
        return {
            'full_export': full_path,
            'summary_export': summary_path,
            'count': len(contractors)
        }
    
    async def run_complete_pipeline(self, target_count: int = None, export_limit: int = None):
        """Run the complete processing and export pipeline"""
        start_time = datetime.now()
        logger.info(f"Starting complete processing pipeline at {start_time}")
        
        try:
            # Step 1: Process contractors
            processed_count = await self.process_contractors(target_count=target_count)
            
            # Step 2: Export processed contractors
            export_result = await self.export_processed_contractors(limit=export_limit)
            ...
            logger.info(f"  - Contractors processed: {processed_count}")
            logger.info(f"  - Contractors exported: {export_result['count'] if export_result else 0}")
            
            if export_result:
                logger.info(f"  - Export files:")
                logger.info(f"    * Full: {export_result['full_export']}")
                logger.info(f"    * Summary: {export_result['summary_export']}")
            
            return {
                'processed_count': processed_count,
                'exported_count': export_result['count'] if export_result else 0,
...
async def main():
    ...
    parser.add_argument('--count', '-c', type=int, default=5000,
                       help='Number of contractors to process (default: 5000)')
    parser.add_argument('--batch-size', '-b', type=int, default=None,
                       help='Batch size for processing (default: from config)')
    parser.add_argument('--export-limit', '-e', type=int, default=None,
                       help='Limit number of contractors to export (default: all processed)')
    parser.add_argument('--process-only', '-p', action='store_true',
                       help='Only process contractors, skip export')
    parser.add_argument('--export-only', '-x', action='store_true',
                       help='Only export processed contractors, skip processing')
    ...
    try:
        ...
        if args.status:
            ...
            # Export only
            result = await orchestrator.export_processed_contractors(limit=args.export_limit)
            if result:
                print(f"\n✅ Export completed: {result['count']} contractors")
                print(f"Full export: {result['full_export']}")
                print(f"Summary export: {result['summary_export']}")
            else:
            ...
            print(f"\n🎉 PIPELINE COMPLETED!")
            print(f"Processed: {result['processed_count']} contractors")
            print(f"Exported: {result['exported_count']} contractors")
            print(f"Duration: {result['duration']}")
            
            if result['export_files']:
                print(f"\nExport Files:")
                print(f"📋 Full: {result['export_files']['full_export']}")
                print(f"📄 Summary: {result['export_files']['summary_export']}")
