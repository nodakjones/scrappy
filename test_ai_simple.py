#!/usr/bin/env python3
"""
Simple test of AI business name mismatch detection
"""
...
async def test_ai_simple():
    ...
    * Appt
    ...
    business_name_match = service._advanced_business_name_matching(business_name, website_content)
    keyword_match = service._keyword_business_name_matching(business_name, website_content)
    
    print(f"   - Business name match: {business_name_match:.3f}")
    print(f"   - Keyword match: {keyword_match:.3f}")
    
    # Check if "Runland Painting" appears in content
    runland_count = website_content.lower().count("runland painting")
    print(f"   - 'Runland Painting' appears {runland_count} times in content")
    
    # Check if "A TEAM PAINTING" appears in content
    team_count = website_content.lower().count("a team painting")
    print(f"   - 'A TEAM PAINTING' appears {team_count} times in content")
    
    print(f"\n📋 Analysis:")
    print(f"   - Expected: Very low confidence due to business name mismatch")
    print(f"   - 'Runland Painting' is prominent in content")
    print(f"   - 'A TEAM PAINTING' is not found in content")
    print(f"   - This should result in very low confidence (0.0-0.2)")
